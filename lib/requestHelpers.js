 ...
 * Transform "$null" string values into real null value
 * we need that because the client is not able to send real null values
 * @param {object} _objectToParse to browse in order to parse values
 * @returns {void}
 */
function parseValues(_objectToParse) {
    if (_objectToParse) {
        Object.keys(_objectToParse)
            .filter((_key) => _objectToParse[_key] === "$null")
            .forEach((_key) => {
                _objectToParse[_key] = null;
            });

        Object.keys(_objectToParse)
            .filter((_key) => _objectToParse[_key] && typeof _objectToParse[_key] === "object")
            .forEach((_key) => {
                parseValues(_objectToParse[_key]);
            });
    }
}
