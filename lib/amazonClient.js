...
module.exports.s3GetObject = function s3GetObject(app, bucket, fileKey, region) {
    ...
    var request = https.request(options, function (res) {
        ...
        res.on("end", function() {
            if (res.statusCode == 200) {
                ...
                var err = new Error("s3GetObject Error: " + res.statusCode + " bucket="+bucket + " fileKey=" + fileKey + " region=" + region + "\n" + data);
                deferred.reject(err);
            }
        });
...
module.exports.s3PutObject = function s3PutObject(app, region, bucket, fileKey, contentType, fileContent, ACL, cacheControl) {
    var deferred = q.defer();
    ...
    var req = https.request(options, function(res) {
        ...
        res.on('end', function() {
            if (res.statusCode != 200) {
                var err = new Error("s3PutObject Error: " + res.statusCode + " bucket=" + bucket + " fileKey=" + fileKey + " region=" + region + "\n" + data);
                deferred.reject(err);
            } else {
                deferred.resolve();
            }
