...
function textToExtract($txt) {
    ...
    if (cleanedTxt) {
        ...
        if (cleanedTxt.length > 200) {
            cleanedTxt = cleanedTxt.substring(0, 200 - 3 + 1);

            indexOfLastSpace = cleanedTxt.lastIndexOf(" ");
            if (indexOfLastSpace >= 0) {
                cleanedTxt = `${cleanedTxt.substring(0, indexOfLastSpace)}...`;
            }
        }
    }
...
function sendToDest($to, $outputConfig, $targetObjectId, smtpTransport, asyncValue, mailSubject, action, titleValues, contentValues, customValues, date) {
        ...
        i18n.pSetLanguage($to.language || "fr").then(() => {
            ...
            _createHtmlTemplate(
                ...
                return pLogMail(mailOptions, $to.id, mailSubject, $targetObjectId).then(($mail) => {
                    ...
                    if (!mailOptions.to) {
                        mailError(new Error("Cannot send email without mail destinataires"), mailOptions, $mail);
                    } else {
                        smtpTransport.sendMail(mailOptions, (error) => {
                            //smtpTransport.close();
                            if (error) {
                                ...
                                $mail.save(($$error) => {
                                    if ($$error) {
                                        console.log("saving the log mail when an error during the send occured went wrong", $$error);
                                    }
                                });
    ...
    Q.all([
        ...
        if (Array.isArray(to)) {
            to.forEach((_dest, _index) => {
                setTimeout(() => {
                    sendToDest(_dest, $outputConfig, $targetObjectId, smtpTransport, asyncValue, mailSubject, action, titleValues, contentValues, customValues, date, $outputConfig);
                }, 75 * _index);
            });
        } else {
            sendToDest(to, $outputConfig, $targetObjectId, smtpTransport, asyncValue, mailSubject, action, titleValues, contentValues, customValues, date, $outputConfig);
        }
    }).catch((error) => {
